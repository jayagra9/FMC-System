...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    if (empty($data['vessel_name'])) {
        echo "<script>alert('Vessel Name is required');window.location.href='add_silent_vessel.php';</script>";
        exit();
    }

    try {
        $stmt = $pdo->prepare("
            INSERT INTO silent_vessels (
                vessel_name, owner_name, owner_contact_number, relevant_harbour,
                owner_information_date, owner_informed, sms_to_owner,
                date_to_investigate, comment, remarks,
        ...
        echo "<script>alert('Error: $msg');window.location.href='add_silent_vessel.php';</script>";
        exit();
    }
}
?>
...
<input name="vessel_name" required>
</div>
<div class="form-group">
...
<input name="owner_contact_number" id="owner_contact_number">
</div>
<div class="form-group">
<label>Relevant Harbour</label>
<input name="relevant_harbour">
</div>
</div>
</fieldset>
...
<input type="date" name="owner_information_date">
</div>
<div class="form-group">
...
<input type="date" name="date_to_investigate">
</div>
<div class="form-group">
<label>Comment</label>
<input name="comment">
