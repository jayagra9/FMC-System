...
try {
    $base_sql = "SELECT dv.*, uc.username AS created_by_username, uu.username AS updated_by_username
    ...
    if ($search_query) {
        $base_sql .= " AND (dv.vessel_name LIKE :s OR dv.owner_name LIKE :s OR dv.address LIKE :s OR dv.username LIKE :s OR dv.remark LIKE :s)";
        $params[':s'] = "%$search_query%";
    }

    if ($date_from && $date_to) {
        $base_sql .= " AND dv.date BETWEEN :df AND :dt";
        $params[':df'] = $date_from . ' 00:00:00';
        $params[':dt'] = $date_to . ' 23:59:59';
    } elseif ($date_from) {
        $base_sql .= " AND dv.date >= :df";
        $params[':df'] = $date_from . ' 00:00:00';
    } elseif ($date_to) {
        $base_sql .= " AND dv.date <= :dt";
        $params[':dt'] = $date_to . ' 23:59:59';
    }

    $countSql = preg_replace('/SELECT\s+dv\.,.*/s', 'SELECT COUNT(*)', $base_sql);
    $countStmt = $pdo->prepare($countSql);
    $countStmt->execute($params);
    $total = (int)$countStmt->fetchColumn();
    $total_pages = max(1, ceil($total / $per_page));

    $sql = $base_sql . " ORDER BY dv.created_at DESC LIMIT :limit OFFSET :offset";
...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    header('Content-Disposition: attachment; filename="admin_distress_vessels_' . date('Ymd') . '.xls"');
    echo "<table border=1>";
    $headers = ['ID','Date','Vessel Name','Owner Name','Contact Number','Address','Status','Speed','Position','DateTime Detection','Distance Last Position','Notes','Remark','Departure Form','Voyage','Entered By','Created By','Created At','Updated By','Updated At'];
    echo '<tr>';
    ...
    foreach ($rows as $r) {
        ...
        echo '<td>' . (!empty($r['date']) ? (new DateTime($r['date']))->format('Y-m-d') : '') . '</td>';
        echo '<td>' . htmlspecialchars($r['vessel_name'] ?? '') . '</td>';
        echo '<td>' . htmlspecialchars($r['owner_name'] ?? '') . '</td>';
        echo '<td>' . htmlspecialchars($r['contact_number'] ?? '') . '</td>';
        echo '<td>' . htmlspecialchars($r['address'] ?? '') . '</td>';
        echo '<td>' . htmlspecialchars($r['status'] ?? '') . '</td>';
        echo '<td>' . htmlspecialchars($r['speed'] ?? '') . '</td>';
        echo '<td>' . htmlspecialchars($r['position'] ?? '') . '</td>';
        echo '<td>' . (!empty($r['date_time_detection']) ? (new DateTime($r['date_time_detection']))->format('Y-m-d H:i:s') : '') . '</td>';
        echo '<td>' . htmlspecialchars($r['distance_last_position'] ?? '') . '</td>';
        echo '<td>' . htmlspecialchars($r['notes'] ?? '') . '</td>';
        echo '<td>' . htmlspecialchars($r['remark'] ?? '') . '</td>';
        echo '<td>' . htmlspecialchars($r['departure_form'] ?? '') . '</td>';
        echo '<td>' . htmlspecialchars($r['voyage'] ?? '') . '</td>';
...
<head>
    ...
    <title>Admin — Distress Vessel Logs</title>
...
<div class="main">
    <h1><i class="fas fa-life-ring"></i> Distress Vessel Records</h1>
    <?php if (!empty($error)): ?><div class="error"><?php echo htmlspecialchars($error); ?></div><?php endif; ?>

    <?php if (empty($rows)): ?>
        <div class="no-data"><i class="fas fa-inbox"></i> No distress vessel records found.</div>
    <?php else: ?>
        <div class="card-list">
            <?php foreach ($rows as $r): ?>
                <div class="item">
                    <div class="left">
                        ...
                        <div class="meta"><strong>IMO:</strong> <?php echo htmlspecialchars($r['vessel_imo'] ?? ''); ?></div>
                        <div class="meta"><strong>Contact:</strong> <?php echo htmlspecialchars($r['contact_number'] ?? ''); ?></div>
                        <div class="meta small">Location: <?php echo htmlspecialchars($r['last_known_location'] ?? ''); ?></div>
                ...
                <?php
                    try {
                        $s = $pdo->prepare("SELECT changed_at, action, changed_by, changes FROM activity_audit WHERE table_name = 'distress_vessels' AND record_id = ? ORDER BY changed_at DESC LIMIT 6");
                        $s->execute([$r['id']]);
                    ...
                    <div class="audit-list">
                        ...
                        <?php endforeach; ?>
                        <div style="margin-top:6px;"><a href="#" class="btn" data-id="<?php echo (int)$r['id']; ?>" onclick="showFullHistory(event, '<?php echo 'distress_vessels'; ?>', <?php echo (int)$r['id']; ?>)">View full history</a></div>
                    </div>
                <?php endif; ?>
            <?php endforeach; ?>
        </div>
...
document.querySelectorAll('.history').forEach(b=>b.addEventListener('click', function(){ const id=this.dataset.id; showFullHistory(null,'distress_vessels', id); }));
</script>
</body>
</html>