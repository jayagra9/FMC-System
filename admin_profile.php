...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['profile_picture'])) {
    ...
    // Log incoming POST/file data for debugging
    $logDir = __DIR__ . DIRECTORY_SEPARATOR . 'uploads';
    if (!is_dir($logDir)) {@mkdir($logDir, 0755, true);} 
    $logFile = $logDir . DIRECTORY_SEPARATOR . 'upload_debug.log';
    $debugEntry = date('[Y-m-d H:i:s] Incoming upload attempt by user_id=' . ($_SESSION['user_id'] ?? 'unknown') . "\n");
    $debugEntry .= "FILES keys: " . implode(',', array_keys($_FILES)) . "\n";
    $debugEntry .= "File error: " . ($file['error'] ?? 'n/a') . ", size=" . ($file['size'] ?? 'n/a') . ", tmp_name=" . ($file['tmp_name'] ?? 'n/a') . "\n";
    @file_put_contents($logFile, $debugEntry, FILE_APPEND);
...
<head>
    ...
    <title>Admin Settings - FMC Fisheries</title>
    ...
    <style>
        * {
        ...
        /* Sidebar styles are provided by sidebar.php include */
            ...
            <?php else: ?>
                <div class="profile-card">
                    <div class="profile-body">
                        ...
                        <a href="login.php" class="btn btn-primary" style="max-width: 300px; margin: 20px auto 0;">
